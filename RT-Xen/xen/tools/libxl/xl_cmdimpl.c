    ...
    int domid)
{
    ...
    for( i = 0; i < scinfo.rt.num_vcpus; i++ )
    {
		...
		printf("\n");
		printf("\t \t \t \t Budget Low \t Budget High \t Criticality \n");
		printf("\t \t \t \t %9"PRIu64" \t %9"PRIu64" \t %4d \n",
            scinfo.rt.vcpus[i].budget_low,
            scinfo.rt.vcpus[i].budget_high,
			scinfo.rt.vcpus[i].criticality_vcpu);
    }
    free(domname);
...
int main_sched_rt(int argc, char **argv)
{
    ...
    int budget_low = 4, opt_bl = 0;
    int budget_high = 6, opt_bh = 0;
    int vcpu_index = 0, opt_v = 0;
    int criticality_vcpu = 0, opt_l = 0;
    int opt, rc;
    static struct option opts[] = {
        {"domain", 1, 0, 'd'},
        {"period", 1, 0, 'p'},
        ...
        {"vcpu", 1, 0, 'v'},
        {"cpupool", 1, 0, 'c'},
        {"criticality_vcpu", 1, 0, 'l'},
        COMMON_LONG_OPTS,
        {0, 0, 0, 0}
    };

    SWITCH_FOREACH_OPT(opt, "d:p:b:B:v:c:l:h", opts, "sched-rt", 0) {
    case 'd':
        dom = optarg;
        break;
    case 'p':
        ...
        break;
    case 'b':
        budget_low = strtol(optarg, NULL, 10);
        opt_bl = 1;
        break;
    case 'B':
        budget_high = strtol(optarg, NULL, 10);
        opt_bh = 1;
        break;
    case 'v':
    ...
    case 'c':
        ...
        break;
    case 'l':
        criticality_vcpu = strtol(optarg, NULL, 10);
        opt_l = 1;
        break;
    }

    if (cpupool && (dom || opt_p || opt_bl || opt_bh || opt_v || opt_l)) {
        fprintf(stderr, "Specifying a cpupool is not allowed with other options.\n");
        return 1;
    }
    if (!dom && (opt_p || opt_bl || opt_bh || opt_v || opt_l)) {
        fprintf(stderr, "Must specify a domain.\n");
        return 1;
    }
    if ( (opt_v || opt_p || opt_bl) && (opt_p + opt_bl + opt_v + opt_l != 4) ) {
        fprintf(stderr, "Must specify vcpu, period, budget low and criticality\n");
        return 1;
    }

    if(opt_l && criticality_vcpu == 1){
        if(!(opt_bh)){
            fprintf(stderr, "High criticality, Must specify budget_high \n");
            return 1;
 	   }
	}
    ...
    if (!dom) { /* list all domain's rt scheduler info */
        ...
        uint32_t domid = find_domain(dom);
        if (!opt_p && !opt_bl && !opt_v) { /* output rt scheduler info */
            ...
            scinfo.rt.budget_low = budget_low;
            scinfo.rt.budget_high = budget_high;
            scinfo.rt.criticality_vcpu = criticality_vcpu;
