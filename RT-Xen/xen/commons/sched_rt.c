...
#define LOW_CRITICALITY			(uint16_t)0
#define HIGH_CRITICALITY		(uint16_t)1

#define NORMAL_MODE				(uint16_t)0
#define	BAILOUT_MODE			(uint16_t)1
#define RECOVERY_MODE			(uint16_t)2

static s_time_t BailFund 	= 	(MICROSECS(0));
...
struct rt_private {
	...
	uint16_t sys_mode;		/* Bailout System Mode */
	s_time_t sched_last;
};
...
rt_dump_vcpu(struct rt_vcpu *svc)
{
    ...
    printk("\n #------- rt_dump_vcpu function is called -------# \n");
    ...
    printk("[%5d.%-2d] cpu %d, Period %"PRId64", Budget_low %"PRId64", \
			Budget_high %"PRId64", Criticality %d, \
			cur_b=%"PRId64" cur_d=%"PRId64" \
			last_start=%"PRId64" onR=%d runnable=%d \
			cpu_hard_affinity=%s \n",
...
rt_dump(const struct scheduler *ops)
{
    ...
    printk("#------- rt_dump function is called -------# \n");
...
__runq_insert(const struct scheduler *ops, struct rt_vcpu *svc)
{
    ...
    printk("\n \n \n VCPU %d invoked runq insert function.......\n",svc->vcpu->vcpu_id);
    rt_dump_vcpu(svc);
    /* Debug only */
    if ( __vcpu_on_runq(svc) )
    {
    ...
    printk("added VCPU %d into the RUNQ.......\n",svc->vcpu->vcpu_id);
}
...
rt_init(struct scheduler *ops)
{
	...
	printk("#------- rt_init function is called -------# \n");
	printk("System Starts in NORMAL_MODE %d \n", prv->sys_mode);
	...
	/* Bailout: Initialised Mode of the System to Normal Mode*/
	prv->sys_mode = NORMAL_MODE;
...
static void 
fund_manager(const struct scheduler *ops, struct rt_vcpu *svc, s_time_t now) {

	struct rt_private *prv = RT_PRIV(ops);
	
	printk("\n \n \n #------- fund_manager function is called -------# \n");
	printk("Fund manager called at %ld -- bailfund is %ld\n", now, BailFund);

	/* Normal mode to bailout mode */

	if( (prv->sys_mode == NORMAL_MODE) && 
		(svc->criticality_vcpu == HIGH_CRITICALITY) ) {

		if(svc->cur_budget <= 0) {
			prv->sys_mode = BAILOUT_MODE;

			printk("Old BailFund = %"PRId64" \n", BailFund);

			BailFund = BailFund + ( (svc->budget_high - svc->budget_low) * 1000 );
			svc->cur_budget = (svc->budget_high - svc->budget_low) * 1000;

			printk("New BailFund = %"PRId64" and vcpu cur_budget = %ld \n", BailFund, svc->cur_budget);
			printk("NORMAL_MODE to BAILOUT_MODE is Switched \n");
		}
		
		return;
	}

	/* Bailout mode to Recovery mode */
	
    if( (prv->sys_mode == BAILOUT_MODE) && 
		(svc->criticality_vcpu == HIGH_CRITICALITY) ) {

		if( BailFund <= (s_time_t)0 ) {
			BailFund = (MICROSECS(0));
			prv->sys_mode = RECOVERY_MODE; /* vcpu crossed C(LO) and BF = 0*/

			printk("BAILOUT_MODE to RECOVERY_MODE is Switched and BF = 0 \n");
		}
		
		return;
	}

	/* Recovery mode to Bailout mode */
	
	if( (prv->sys_mode == RECOVERY_MODE) && 
		(svc->criticality_vcpu == HIGH_CRITICALITY) ) {

		if(svc->cur_budget <= 0) {
			prv->sys_mode = BAILOUT_MODE; /* vcpu(HI) crossed C(LO) */
			BailFund = (svc->budget_high - svc->budget_low) * 1000;
			svc->cur_budget = (svc->budget_high - svc->budget_low) * 1000;

			printk("RECOVERY_MODE to BAILOUT_MODE is Switched, \n \
					cur_budget < C(LO) = %ld < %"PRId64" \n", svc->cur_budget, RT_DEFAULT_BUDGET_LOW);
		}
...
burn_budgets(const struct scheduler *ops, struct rt_vcpu *svc, s_time_t now) 
{
    ...
    printk("\n \n \n #------- burn_budgets function is called -------# \n");
    //printk("now = %"PRId64" and System Mode = %d \n", now, prv->sys_mode);

	printk("Entered burn_budget at %"PRId64" \n \
			current budget of vcpu is %ld \n \
			criticality_vcpu of vcpu is %d \n \
			Last start time of vcpu is %"PRId64" \n \
			Current deadline of vcpu is %"PRId64" \n \
			now time is %"PRId64" \n \
			vcpu ID = %d \n",
	...
	printk("\n delta(now-cur_deadline) is %"PRId64"\n",delta);
    if ( delta >= 0 ) 
    {	printk("Vcpu has missed the deadline \n");
        count = ( delta/MICROSECS(svc->period) ) + 1;
        svc->cur_deadline += count * MICROSECS(svc->period);
		
		if(prv->sys_mode == BAILOUT_MODE) {
			
			if(svc->criticality_vcpu == HIGH_CRITICALITY) {
    ...
    delta = now - svc->last_start;
		printk("now is %"PRId64" \n \
			and svc->last_start is %"PRId64" \n \
			and now - svc->last_start is %"PRId64" \n \
			and svc->cur_budget is %ld \n",
			now, svc->last_start, delta, svc->cur_budget);
    /* 
    ...
    if ( svc->cur_budget == 0 ) 
 	...
 	printk("#---------------Properties of Current Burning Vcpu :: \n");
	printk("Period 	= %"PRId64" \n", 	svc->period);
    printk("Cur Budget 	= %ld \n", 		svc->cur_budget);
    printk("Budget Low 	= %"PRId64" \n", 	svc->budget_low);
    printk("Budget High = %"PRId64" \n", 	svc->budget_high);
    printk("Criticality = %d \n", 		svc->criticality_vcpu);
	printk("#---------------Burning budget...\n");

	printk("SVC current budget before burning is %ld\n",svc->cur_budget);
    svc->cur_budget -= delta;
    printk("SVC current budget after burning is %ld where delta is %ld \n",svc->cur_budget,delta);

    fund_manager(ops, svc, now);
    if ( svc->cur_budget < 0 ) 
        svc->cur_budget = 0;
}
...
__runq_pick(const struct scheduler *ops, cpumask_t mask)
{
    ...
    printk("\n \n \n #------- runq_pick function is called -------# \n");
    //printk("System Mode = %d \n", prv->sys_mode);

    list_for_each(iter, runq) 
    {
        iter_svc = __runq_elem(iter);
	
	rt_dump_vcpu(iter_svc);
	printk("\n");
	    ...
	    if(prv->sys_mode == BAILOUT_MODE && 
			iter_svc->criticality_vcpu == LOW_CRITICALITY)
		    continue;
...
__repl_update(const struct scheduler *ops, s_time_t now)
{
	...
	printk("\n \n \n #------- repl_update function is called -------# \n");
	//printk("now = %"PRId64" and System Mode = %d \n", now, prv->sys_mode);
...
//	rt_dump(ops);
    ...
    list_for_each_safe(iter, tmp, runq) 
    {
		...
		/*Updating BailFund for a release of LO_crit task in BAILOUT_MODE */
		/*Scan the runq and check for any low crit vcpu released between 'NOW' and previous time instance when scheduler was called*/
		if(prv->sys_mode == BAILOUT_MODE && 
			svc->criticality_vcpu == LOW_CRITICALITY && 
			svc->cur_budget > 0) {    //In bailout mode,In runq, for low crit vcpu having some budget left 

			if((svc->cur_deadline - svc->period) <= now || 
				(svc->cur_deadline - svc->period) > prv->sched_last) {  //If release time < now or If release time > previous scheduler called instance
         		BailFund = (s_time_t)( (long)BailFund - svc->cur_budget );
		 	}
...
rt_schedule(const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
	...
	printk("\n \n \n #------- rt_schedule function is Called -------# \n");
	printk("now = %"PRId64" and System Mode = %d \n", now, prv->sys_mode);
    ...
    if ( tasklet_work_scheduled ) 
    {
		...
		printk("#------- snext = Idle_vcpu [tasklet_work_scheduled == 1] -------# \n");
    } 
    else 
    {
        cpumask_t cur_cpu;
        ...
        if ( !is_idle_vcpu(current) &&
			...
			printk("#------- Current Running Vcpu before calling __runq_pick function -------# \n");
			rt_dump_vcpu(scurr);

			snext = scurr;

			printk("#------- Deciding What to Pick ??? -------# \n");
			rt_dump_vcpu(snext);
		}
    }
    ...
    rt_dump_vcpu(snext);
    if(is_idle_vcpu(snext->vcpu)){
		/* Switching System Mode to NORMAL_MODE and BailFund = 0*/
		prv->sys_mode = NORMAL_MODE;
		BailFund = (MICROSECS(0));
    }
...
runq_tickle(const struct scheduler *ops, struct rt_vcpu *new)
{
	...
	printk("#------------runq_tickle condition 1--------------# \n");
        return;
    }
    ...
    for_each_cpu(cpu, &not_tickled) 
    {
	    ...
	    printk("#------------runq_tickle condition 2--------------# \n");
            return;
        }
        iter_svc = RT_VCPU(iter_vc);
        if ( latest_deadline_vcpu == NULL || 
             iter_svc->period > latest_deadline_vcpu->period ){
...
/*    if ( latest_deadline_vcpu != NULL && new->cur_deadline < latest_deadline_vcpu->cur_deadline ) 
    {
		...
		printk("#------------runq_tickle condition 3--------------#\n");
    }
*/    
	return;
}
    ...
    struct xen_domctl_scheduler_op *op)
{
	...
	printk("\n \n \n #------- rt_dom_cntl function is called -------# \n");
    ...
    case XEN_DOMCTL_SCHEDOP_getinfo:
		printk("Case = XEN_DOMCTL_SCHEDOP_getinfo \n");
    ...
    case XEN_DOMCTL_SCHEDOP_putinfo:
		printk("Case = XEN_DOMCTL_SCHEDOP_putinfo \n");
        ...
        list_for_each( iter, &sdom->vcpu ) 
        {
            ...
            { 
				...
				"vcpu_index=%d,period=%"PRId64",budget_low=%"PRId64", \
				budget_high=%"PRId64"\n",
				vcpu_index, op->u.rt.period, op->u.rt.budget_low, 
				op->u.rt.budget_high);
					}
