                               ...
                               libxl_domain_sched_params *scinfo)
{
    ...
    for( i = 0; i < num_vcpus; ++i)
    {
        ...
        scinfo->rt.vcpus[i].budget_low = sdom[i].budget_low;
        scinfo->rt.vcpus[i].budget_high = sdom[i].budget_high;
        scinfo->rt.vcpus[i].index = sdom[i].index;
        scinfo->rt.vcpus[i].criticality_vcpu = sdom[i].criticality_vcpu;
    }
                                               ...
                                               const uint16_t num_vcpus)
{
    ...
    if (vcpu_index == LIBXL_DOMAIN_SCHED_PARAM_VCPU_DEFAULT ||
        ...
        scinfo->rt.budget_low == LIBXL_DOMAIN_SCHED_PARAM_BUDGET_DEFAULT)
    {
        return 1;
    }
    ...
    if (scinfo->rt.budget_low < 1 ||
        scinfo->rt.budget_low > SCHED_RT_VCPU_BUDGET_MAX)
    {
    ...
    if (scinfo->rt.criticality_vcpu < 0 || scinfo->rt.criticality_vcpu > 1)
    {
        LOG(ERROR, "VCPU criticality is not set or out of range, "
                    "valid values are within range from 0(LOW) to 1(HIGH)");
        return 2;
    }
                               ...
                               const libxl_domain_sched_params *scinfo)
{
    ...
    if (rc == 0)
    {
        ...
        sdom.budget_low = scinfo->rt.budget_low;
        sdom.budget_high = scinfo->rt.budget_high;
        sdom.criticality_vcpu = scinfo->rt.criticality_vcpu;
    }
